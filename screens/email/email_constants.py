...
EMAIL_REGEN_TITLE = "For the following generated email, youi must generate a new title. The previously generated title will be attatched to the email but when you generate a new title, make sure it is unique and different than the one provided. For your response, format it as you normally would. Include Subject: followed by the newly generated title. You dont need to return any other content other than that.\n\n"
EMAIL_PRECON = "The reason for this message is to give context, you only need to reply to this message with \"Ok\". The following message with contain the actual prompt in which you will use these details above to generate an email. "
EMAIL_META_SPECIAL = """Dont add any extra details at the start of the message. Also for readability, feel free to add empty lines between text.
